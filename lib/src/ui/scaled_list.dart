...
/// A Horizontal List view With Lots of modification including the a scaled
/// current item, curved custom painting and a dots indicator below it.
/// it makes the widget of item builder responsive to
/// the device screen's width and height
/// You can also set the  relative width , margin width , height of selected
/// and unselected Card to the entire screen
class ScaledList extends StatefulWidget {
  const ScaledList({
    ...
    this.unSelectedCardHeightRatio = 0.3,
          ...
          cardWidthRatio + marginWidthRatio >= 0.5,
          'Card width + margin width should exceed 0.5 of the screen',
        ),
        assert(
          selectedCardHeightRatio > unSelectedCardHeightRatio,
          'SelectedCardHeight should always exceed the unSelectedCardHeight '
          'to show desire effect',
        ),
        super(key: key);

  /// return a widget to build and give you the index of that widget
  /// and the selected index which appear currently on the screen
  ...
  /// the margin width between cards relative to the device screen width
  /// or the parent width the default value is 0.1
  final double marginWidthRatio;

  /// the selected card height relative to the device screen width
  /// or the parent width the default value is 0.4
  final double selectedCardHeightRatio;

  /// the unSelected card height relative to the device screen width
  /// or the parent width the default value is 0.3
  final double unSelectedCardHeightRatio;

  @override
  ScaledListState createState() => ScaledListState();
}

class ScaledListState extends State<ScaledList> {
  ...
  void initState() {
    ...
    final fullCardWidth = widget.cardWidthRatio + widget.marginWidthRatio;
        ...
        return Column(
          ...
          children: [
            ...
            if (widget.showDots) ...[
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: List.generate(widget.itemCount, buildDot),
              ),
            ]
          ],
        );
